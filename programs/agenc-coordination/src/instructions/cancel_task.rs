...
fn process_cancel_task_impl<'info>(
    ...
    let num_triples = remaining_accounts
        .len()
        .checked_div(3)
        .ok_or(CoordinationError::ArithmeticOverflow)?;
    ...
    for i in 0..num_triples {
        ...
        let claim_info = &remaining_accounts[base];
        let worker_info = &remaining_accounts[worker_index];
        let rent_recipient_info = &remaining_accounts[rent_recipient_index];
