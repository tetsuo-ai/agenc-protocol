...
// guard now protects completing non-Collaborative accepts; Collaborative tasks
// deliberately retain multi-submission completion and use terminal cleanup.
//
...
// no-live-submission evidence, plus a still-SUBMITTED Collaborative straggler
// after completion. Rejected counters were already decremented at bounce time;
// Submitted cleanup settles the remaining validation counters.
//
...
async function setupManualTask(
  w,
  {
    mode = 1,
    reviewWindow = 3600,
...
async function validateIx(
  w,
  m,
  s,
  { validatorKp, validatorAgent, workerAgent, workerKp, approved },
) {
  const [vote] = pda([
    enc("task_validation_vote"),
    s.submission.toBuffer(),
    validatorKp.publicKey.toBuffer(),
  ]);
  return makeProgram(validatorKp)
    ...
    .accounts({
      ...
      creator: w.buyer.publicKey,
      workerAuthority: workerKp.publicKey,
      reviewer: validatorKp.publicKey,
      tokenEscrowAta: null,
...
test("F-3a: a completing quorum accept is blocked while a peer submission is live, then settles after the peer is rejected", async () => {
  ...
  const m = await setupManualTask(w, {
    mode: 2,
    validatorQuorum: 1,
    maxWorkers: 2,
  });
  ...
  assert.equal(
    decode(w.svm, "Task", m.task).live_submissions ?? 2,
    2,
    "two live submissions",
  );
  ...
  expectFail(
    send(
      w.svm,
      await validateIx(w, m, sA, {
        validatorKp: v.kp,
        validatorAgent: v.agentPda,
        workerAgent: a.agentPda,
        workerKp: a.kp,
        approved: true,
      }),
      [v.kp],
  ...
  assert.ok(
    ...
    "task NOT completed by the blocked accept",
  );
  ...
  expectOk(
    send(
      w.svm,
      await validateIx(w, m, sB, {
        validatorKp: v.kp,
        validatorAgent: v.agentPda,
        workerAgent: b.agentPda,
        workerKp: b.kp,
        approved: false,
      }),
      [v.kp],
  ...
  assert.ok(
    isClosed(w.svm, sB.claim),
    "rejected peer's claim closed by the reject branch",
  );
  ...
  assert.ok(
    decode(w.svm, "Task", m.task).status.Completed !== undefined,
    "task Completed once sole-live",
  );
  assert.ok(
    Number(w.svm.getBalance(a.kp.publicKey)) > aBefore,
    "worker A paid on the completing accept",
  );
});
...
test("F-3b: a bounced claim stranded by a peer's completing accept is reclaimed via Rejected-submission evidence", async () => {
  ...
  expectOk(
    ...
    "bounce A via request_changes",
  );
  assert.equal(
    decode(w.svm, "TaskSubmission", sA.submission).status.Rejected !==
      undefined,
    true,
    "A's submission bounced to Rejected",
  );
  ...
  expectOk(
    send(
      ...
      await w.buyerProg.methods
        ...
        .accounts({
          ...
          escrow: m.escrow,
          taskValidationConfig: m.validation,
          taskSubmission: sB.submission,
          worker: b.agentPda,
          protocolConfig: w.protocolPda,
          treasury: w.admin.publicKey,
          creator: w.buyer.publicKey,
          workerAuthority: b.kp.publicKey,
          operator: null,
          referrer: null,
          hireRecord: null,
    ...
    "accept B (completing, sole-live)",
  );
  assert.ok(
    decode(w.svm, "Task", m.task).status.Completed !== undefined,
    "task Completed by B's accept",
  );
  assert.equal(
    decode(w.svm, "Task", m.task).current_workers,
    1,
    "A's bounced claim still counted",
  );
  ...
  assert.ok(
    isClosed(w.svm, sA.submission),
    "A's bounced submission closed (rent returned)",
  );
  assert.equal(
    decode(w.svm, "Task", m.task).current_workers,
    0,
