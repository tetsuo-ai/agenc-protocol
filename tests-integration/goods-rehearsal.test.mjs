// GOODS launch-control REHEARSAL (litesvm, runs the compiled .so).
//
...
test("REHEARSAL: dark -> atomic-stamped current -> unpause with mask -> sale event decodes", async () => {
  ...
  // The atomic-release suite exercises stamp_release_surface itself. Model its
  // exact launch-control post-image here (CURRENT + paused), then exercise the
  // distinct, real multisig unpause action used after the reviewed stamp.
  await setSurfaceRevision(w.svm, REV_CURRENT);
  await setProtocolPaused(w.svm, true);
  w.svm.expireBlockhash();
  expectOk(await stamp(false, LIVE_MASK, KEEP_REV), "unpause atomically stamped release");
  assert.equal(getSurfaceRevision(w.svm), REV_CURRENT, "goods surface LIVE at CURRENT");
  const cfg = readConfig(w);
  assert.equal(cfg.disabled_task_type_mask, LIVE_MASK, "UNPAUSE: intended mask stored byte-identical");
  assert.equal(cfg.protocol_paused, false, "UNPAUSE: current stamped release made live");
...
test("REHEARSAL (hazard): a launch-control update that forgets the mask RESETS it", async () => {
  // Documents the launch-control trap: passing mask=0 while changing another
  // field clears a live mask. Callers should use KEEP_MASK when preserving it.
  const w = await freshWorld({ price: 1_000_000, moderationEnabled: false });
  const [protocolPda] = pda([enc("protocol")]);
  const owner2 = Keypair.generate();
  w.svm.airdrop(owner2.publicKey, BigInt(10e9));
  ...
  await setProtocolPaused(w.svm, true);
  w.svm.expireBlockhash();
  expectOk(await stamp(false, LIVE_MASK, KEEP_REV), "unpause with mask set");
  assert.equal(readConfig(w).disabled_task_type_mask, LIVE_MASK);
  // Forget to preserve the mask on a later update -> it is CLEARED.
  w.svm.expireBlockhash();
  expectOk(await stamp(false, 0, KEEP_REV), "update but drop the mask");
  assert.equal(readConfig(w).disabled_task_type_mask, 0, "the mask was reset — pass KEEP_MASK to preserve it");
});
...
test("F-18: KEEP sentinels preserve the mask + revision instead of silently resetting them", async () => {
  ...
  await setSurfaceRevision(w.svm, REV_CURRENT);
  await setProtocolPaused(w.svm, true);
  expectOk(await stamp(false, LIVE_MASK, KEEP_REV), "unpause current release with mask set");
  // Preserve both fields on a subsequent update — the stale-read hazard is closed.
  w.svm.expireBlockhash();
  expectOk(await stamp(false, KEEP_MASK, KEEP_REV), "preserve mask and current revision");
  assert.equal(getSurfaceRevision(w.svm), REV_CURRENT, "revision preserved");
  ...
  assert.equal(getSurfaceRevision(w.svm), REV_CURRENT, "revision PRESERVED by the KEEP sentinel (F-18)");
});
