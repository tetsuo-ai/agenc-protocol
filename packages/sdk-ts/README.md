...
## Protocol multisig admin builders

Protocol-config admin and migration facade builders take `multisigSigners`, an
ordered array of `TransactionSigner` wallet keys. The facade appends them after
the instruction's named accounts as read-only signer remaining accounts and
rejects duplicate addresses within that array. A multisig owner used as the
instruction's named `authority` must also be included in `multisigSigners`: Rust
counts approvals only from the remaining-account suffix, so the repeated account
meta is intentional. Rust enforces system ownership, M-of-N membership, and the
configured threshold on-chain.
...
| `npm run sdk:drift`      | Hash generated paths/bytes, regenerate, and fail if the tree changes (stale vs IDL). |
