// Minimal litesvm harness for the worker e2e — runs the ACTUAL compiled
// agenc-coordination program in-process (litesvm 1.1.0, kit-native).
// Mirrors packages/sdk-ts/tests-e2e/harness.ts (the compact pieces this suite
// needs); ProtocolConfig/ModerationConfig are INJECTED via svm.setAccount
// because their real initializers need an upgradeable ProgramData account
// litesvm doesn't model.
...
export async function seedProtocolConfig(
  ...
  const data = getProtocolConfigEncoder().encode({
    ...
    // The e2e suite runs the FULL-surface .so, so the injected config must
...
/** Seed a ModerationConfig account (the job-spec publish gate requires it). */
