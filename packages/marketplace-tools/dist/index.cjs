...
__export(src_exports, {
  A2A_AGENC_EXTENSION_URI: () => A2A_AGENC_EXTENSION_URI,
  A2A_AGENC_PROTOCOL_BINDING: () => A2A_AGENC_PROTOCOL_BINDING,
  A2A_SCHEMA_VERSION: () => A2A_SCHEMA_VERSION,
  AGENT_CARD_SCHEMA_VERSION: () => AGENT_CARD_SCHEMA_VERSION,
  MarketplaceToolError: () => MarketplaceToolError,
  buildAgentCardManifest: () => buildAgentCardManifest,
  createToolRegistry: () => createToolRegistry,
  getTool: () => getTool,
  indexerListingToAgentCard: () => indexerListingToAgentCard,
  listingToAgentCard: () => listingToAgentCard,
  marketplaceToolRegistry: () => marketplaceToolRegistry,
...
var import_marketplace_sdk2 = require("@tetsuo-ai/marketplace-sdk");

// src/project.ts
var import_marketplace_sdk = require("@tetsuo-ai/marketplace-sdk");
var { decodeListingName, decodeListingCategory, decodeListingTags } = import_marketplace_sdk.values;
...
function listingStateName(state) {
  return import_marketplace_sdk.ListingState[state] ?? String(state);
}
function taskStatusName(status) {
  return import_marketplace_sdk.TaskStatus[status] ?? String(status);
}
...
// src/tools/readonly.ts
var { getAgentTrackRecord } = import_marketplace_sdk2.facade;
...
var getListing = defineTool({
  ...
  async handler(args, ctx) {
    ...
    const maybe = await (0, import_marketplace_sdk2.fetchMaybeServiceListing)(rpc, args.pda);
...
var getTask = defineTool({
  ...
  async handler(args, ctx) {
    ...
    const maybe = await (0, import_marketplace_sdk2.fetchMaybeTask)(rpc, args.pda);
    if (!maybe.exists) return { task: null };
    let jobSpecPinned = null;
    if (maybe.data.status === import_marketplace_sdk2.TaskStatus.Open) {
      const [jobSpecPda] = await (0, import_marketplace_sdk2.findTaskJobSpecPda)({
        task: maybe.address
      });
      const jobSpec = await (0, import_marketplace_sdk2.fetchMaybeTaskJobSpec)(rpc, jobSpecPda);
...
var import_marketplace_sdk3 = require("@tetsuo-ai/marketplace-sdk");
...
function assertListingMetadata(name, tags, tool) {
  try {
    import_marketplace_sdk3.values.encodeListingName(name);
    import_marketplace_sdk3.values.encodeListingTags(tags);
  } catch (error) {
    throw new MarketplaceToolError(
...
var prepareCreateServiceListing = defineTool({
  ...
  inputSchema: {
    ...
    properties: {
      ...
      category: {
        ...
        enum: import_marketplace_sdk3.values.LISTING_CATEGORIES
      },
      tags: {
        type: "array",
        minItems: 0,
  ...
  async handler(args) {
    ...
    if (!import_marketplace_sdk3.values.isListingCategory(args.category)) {
      throw new MarketplaceToolError(
        "BAD_CATEGORY",
    ...
    const ix = await import_marketplace_sdk3.facade.createServiceListing({
...
var prepareHire = defineTool({
  ...
  async handler(args) {
    ...
    const buyer = (0, import_kit.createNoopSigner)(args.buyer);
    const input = {
    ...
    const ix = await import_marketplace_sdk3.facade.hireFromListing(input);
    return projectInstruction(ix);
  }
});
var prepareHireHumanless = defineTool({
  ...
  async handler(args) {
    ...
    if (args.referrerFeeBps !== void 0)
      input.referrerFeeBps = args.referrerFeeBps;
    const ix = await import_marketplace_sdk3.facade.hireFromListingHumanless(input);
    return projectInstruction(ix);
  }
});
var prepareSetTaskJobSpec = defineTool({
  ...
  async handler(args) {
    const input = {
      task: args.task,
      creator: (0, import_kit.createNoopSigner)(args.creator),
      jobSpecHash: hex322(
        args.jobSpecHash,
        "jobSpecHash",
    ...
    const ix = await import_marketplace_sdk3.facade.setTaskJobSpec(input);
    return projectInstruction(ix);
  }
});
var prepareClaim = defineTool({
  ...
  async handler(args) {
    const authority = (0, import_kit.createNoopSigner)(args.workerAuthority);
    const ix = await import_marketplace_sdk3.facade.claimTaskWithJobSpec({
      task: args.task,
      worker: args.worker,
      authority,
...
var prepareSubmit = defineTool({
  ...
  async handler(args) {
    const authority = (0, import_kit.createNoopSigner)(args.workerAuthority);
    const resultData = args.resultData !== void 0 ? (0, import_kit.some)(
      hexFixed(
        args.resultData,
        RESULT_DATA_BYTES,
        "resultData",
    ...
    const ix = await import_marketplace_sdk3.facade.submitTaskResult({
      task: args.task,
      worker: args.worker,
      authority,
...
var prepareAccept = defineTool({
  ...
  async handler(args) {
    const ix = await import_marketplace_sdk3.facade.acceptTaskResult({
      task: args.task,
      worker: args.worker,
...
var prepareReject = defineTool({
  ...
  async handler(args) {
    const ix = await import_marketplace_sdk3.facade.rejectTaskResult({
      task: args.task,
      claim: args.claim,
      worker: args.worker,
      workerAuthority: args.workerAuthority,
...
var prepareAutoAccept = defineTool({
  ...
  async handler(args) {
    const ix = await import_marketplace_sdk3.facade.autoAcceptTaskResult({
      task: args.task,
      worker: args.worker,
      workerAuthority: args.workerAuthority,
      treasury: args.treasury,
...
var prepareCancel = defineTool({
  ...
  async handler(args) {
    const ix = await import_marketplace_sdk3.facade.cancelTask({
      task: args.task,
      authority: (0, import_kit.createNoopSigner)(args.authority),
...
var prepareClose = defineTool({
  ...
  async handler(args) {
    ...
    const authority = (0, import_kit.createNoopSigner)(args.authority);
    const [creatorCompletionBond] = await (0, import_marketplace_sdk3.findCreatorCompletionBondPda)({
      task,
      creator: authority.address
    });
    const ix = await import_marketplace_sdk3.facade.closeTask({
      task,
      authority,
...
var prepareRateHire = defineTool({
  ...
  async handler(args) {
    const ix = await import_marketplace_sdk3.facade.rateHire({
      task: args.task,
      listing: args.listing,
      buyer: (0, import_kit.createNoopSigner)(args.buyer),
      score: args.score,
      ...args.reviewHash ? {
        reviewHash: hex322(
          args.reviewHash,
...
var import_marketplace_sdk4 = require("@tetsuo-ai/marketplace-sdk");
...
function stateString(state) {
  switch (state) {
    case import_marketplace_sdk4.ListingState.Paused:
      return "paused";
    case import_marketplace_sdk4.ListingState.Retired:
      return "retired";
    case import_marketplace_sdk4.ListingState.Active:
    default:
      return "active";
  }
}
...
function listingToAgentCard(decoded, options = {}) {
  ...
  const tags = import_marketplace_sdk4.values.decodeListingTags(account.tags);
  const specHash = toHex2(account.specHash);
  const priceMint = (0, import_kit2.unwrapOption)(account.priceMint);
  const price = {
  ...
  return {
    ...
    hire: {
      program: String(import_marketplace_sdk4.AGENC_COORDINATION_PROGRAM_ADDRESS),
      listing: listingPda,
      ...
      instruction: `To hire: prepare a humanless hire transaction (buyer wallet, listing=${listingPda}, expectedPrice=${account.price.toString()}, expectedVersion=${account.version.toString()}, listingSpecHash=${specHash}, plus the moderator pubkey whose moderation attestation the hire consumes \u2014 from your attestation service, e.g. attest.agenc.ag GET /v1/info) with the SDK facade, MCP prepare tools, or your operator transaction builder, sign the unsigned transaction locally, and broadcast it. The humanless hire mints a Task + escrow on program ${String(import_marketplace_sdk4.AGENC_COORDINATION_PROGRAM_ADDRESS)}.`
    },
    a2a: {
