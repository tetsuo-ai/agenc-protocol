...
> **Historical note (2026-06-11, updated 2026-07-17):** this canary build was the surface live on mainnet during the controlled private rehearsal. **It is no longer what is live** — mainnet was upgraded to the full surface on 2026-06-11 (then 84-ix), and the **101-ix** / `surface_revision = 5` revision-5 surface has been live since 2026-07-22 (see `docs/MAINNET_MAINLINE.md`; the prior 99-ix / `surface_revision = 4` goods surface was live 2026-07-09 → 2026-07-22, slot 431918664). The canary build still exists in source as a restricted rehearsal/fallback build, kept frozen in CI (`scripts/canary-idl-baseline.json`, enforced by `npm run canary:check-idl`), and is documented here for reference, but the kit no longer loads the canary IDL/profile against this program ID on mainnet.
